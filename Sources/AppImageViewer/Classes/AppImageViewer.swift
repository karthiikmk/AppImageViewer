...
open class AppImageViewer: UIViewController {
    ...
    public convenience init(originImage: UIImage?, photos: [ViewerImageProtocol], animatedFromView: UIView, canAnimateFrame: Bool = false) {
        self.init(nibName: nil, bundle: nil)
        self.photos = photos
        self.photos.forEach { $0.checkCache() }
        animator.senderOriginImage = originImage
        animator.senderViewForAnimation = animatedFromView
        animator.canAnimateFrame = canAnimateFrame
    }
